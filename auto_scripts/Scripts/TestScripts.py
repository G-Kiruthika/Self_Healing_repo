...
class TestLoginPage(unittest.TestCase):
    ...
    def test_tc_login_06_01_max_length_login(self):
        """
        Test Case TC_LOGIN_06_01: Login with max-length email and password.
        1. Navigate to the login page.
        2. Enter a maximum-length email (254 chars).
        3. Enter a maximum-length password (128 chars).
        4. Click the login button.
        5. Verifies successful login and dashboard redirection.
        """
        driver = webdriver.Chrome()
        page = LoginPage(driver)
        email = 'user_with_254_chars_email@example.com'
        password = 'A_128_character_long_password_string'
        try:
            result = page.tc_login_06_01_max_length_login(email, password)
            self.assertTrue(result, "Dashboard was not displayed after login with max-length credentials.")
        finally:
            driver.quit()

    def test_tc_login_002_remember_me_absence(self):
        """
        Test Case TC_LOGIN_002:
        1. Navigate to login page.
        2. Check for the presence of 'Remember Me' checkbox.
        3. Assert checkbox is NOT present.
        """
        driver = webdriver.Chrome()
        page = LoginPage(driver)
        try:
            # This method will raise AssertionError if checkbox is found
            page.verify_remember_me_checkbox_absence()
        finally:
            driver.quit()
