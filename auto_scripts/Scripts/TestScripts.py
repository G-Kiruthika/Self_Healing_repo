...
def test_tc_login_002_invalid_username_valid_password(driver):
    """
    Test Case TC_LOGIN_002: Invalid Username, Valid Password
    ...
    Steps:
        ...
        4. Click Login
        5. Validate error message ('Invalid username or password')
    ...
    login_page.click_login()
    error_message = login_page.get_error_message()
    assert error_message == "Invalid username or password", f"Expected error message not found. Got: {error_message}"
    assert login_page.is_on_login_page(), "User did not remain on login page after invalid login."
    print("TC_LOGIN_002: Passed for invalid username and valid password.")
