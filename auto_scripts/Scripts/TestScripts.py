...
def test_TC_SCRUM96_006_negative_login_api_and_session_validation():
    ...
    # Instantiate LoginPage with dummy driver (not used for API), base_url, and DB config
    login_page = LoginPage(driver=None, base_url=base_url, db_config=db_config)
    username = "validuser"
    email = "validuser@example.com"
    password = "CorrectPass123!"
    wrong_password = "WrongPassword456!"
    # Step 1-3: Run end-to-end workflow
    result = login_page.tc_scrum96_006_workflow(username, email, password, wrong_password)
    assert "registration_response" in result
    assert "login_response" in result
